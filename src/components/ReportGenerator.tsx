...
import { PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, ResponsiveContainer, Tooltip as RechartsTooltip, Legend, LabelList, AreaChart, Area } from 'recharts';
...
export default function ReportGenerator({ isOpen, onClose, data, parseDate }: ReportGeneratorProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Extract top news titles for AI context
      const topNewsTitles = filteredData
        .slice(0, 15)
        .map(row => `- [${row['SENTIMEN']}] ${row['JUDUL']} (${row['NAMA MEDIA']})`)
        .join('\n');
      ...
      const prompt = `
        ...
        3. Struktur laporan harus mencakup:
           ...
           ### II. ANALISIS NARASI DAN REPUTASI DIGITAL
           Bedah makna di balik angka sentimen dan judul berita. Apa tema besar yang sedang berkembang? Apa faktor pendorong sentimen positif? Jika ada sentimen negatif, apa akar permasalahannya secara naratif dan bagaimana dampaknya terhadap citra pariwisata?
           ...
           ### IV. ANALISIS TREN DAN ESKALASI ISU
           Identifikasi apakah ada isu yang berpotensi menjadi krisis atau justru peluang besar yang bisa diakselerasi.
           
           ### V. REKOMENDASI STRATEGIS DAN TINDAK LANJUT
           Berikan minimal 5 rekomendasi konkret yang bersifat manajerial, strategis, dan taktis untuk meningkatkan citra pariwisata Jawa Barat. Gunakan format poin-poin profesional.
          ...
          default: new Header({
            children: [
              new Paragraph({
                ...
                border: {
                  bottom: { color: "1E3A8A", space: 1, style: BorderStyle.SINGLE, size: 6 },
                },
              }),
            ],
          }),
        ...
        <div className="relative z-10 p-12">
          ...
          <div className="grid grid-cols-3 gap-6 mb-12">
            ...
            <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-xl">
              <h3 className="text-sm font-black text-gray-400 mb-6 uppercase tracking-widest text-center">Sentimen Publik</h3>
              <div className="h-[280px] w-full flex items-center justify-center relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-3xl font-black text-gray-900">{reportData.total}</span>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Berita</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-xl">
              <h3 className="text-sm font-black text-gray-400 mb-6 uppercase tracking-widest text-center">Tren Volume Berita</h3>
            ...
            <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-xl">
              <h3 className="text-sm font-black text-gray-400 mb-6 uppercase tracking-widest text-center">Top Destinasi</h3>
              <div className="h-[280px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={reportData.topDestinations} layout="vertical">
                    <XAxis type="number" hide />
                    <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{fontSize: 10, fill: '#1F2937', fontWeight: 700}} width={100} />
        ...
        <div style={{ position: 'fixed', left: '-9999px', top: '0', zIndex: -100 }}>
          ...
          <div ref={pdfRef} className="bg-white" style={{ width: '794px', minHeight: '1123px', padding: '40px' }}>
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              <div className="border border-gray-200 rounded-2xl p-4 bg-white">
                <h3 className="text-sm font-bold text-gray-800 mb-4 uppercase text-center">Tren Volume Berita</h3>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={reportData.volumeData}>
                      ...
                      <Area type="monotone" dataKey="count" stroke="#1D4ED8" strokeWidth={2} fill="#DBEAFE" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </div>
              <div className="border border-gray-200 rounded-2xl p-4 bg-white">
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={reportData.sentimentData || []} cx="50%" cy="50%" innerRadius={50} outerRadius={70} paddingAngle={5} dataKey="value">
                        {(reportData.sentimentData || []).map((entry: any, index: number) => (
                          <Cell key={`cell-${index}`} fill={entry.color} />
                        ))}
                      </Pie>
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              ...
              <div className="border border-gray-200 rounded-2xl p-4 bg-white">
                <h3 className="text-sm font-bold text-gray-800 mb-4 uppercase text-center">Top 5 Media</h3>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={(reportData.topMedia || []).slice(0, 5)} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                      <XAxis type="number" hide />
                      <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fill: '#475569', fontSize: 10 }} width={80} />
          ...
          <div ref={infographicRef} className="bg-slate-950 relative overflow-hidden flex flex-col" style={{ width: '1080px', height: '1920px', padding: '80px' }}>
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              {/* Volume Chart for Infographic */}
              <div className="bg-gray-900/60 border border-gray-800 rounded-[40px] p-12 mb-12 backdrop-blur-sm h-[400px]">
                <h3 className="text-3xl font-bold text-white mb-10 text-center uppercase tracking-widest">Tren Volume Berita</h3>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={reportData.volumeData}>
                    <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{fontSize: 18, fill: '#94A3B8'}} />
                    <YAxis axisLine={false} tickLine={false} tick={{fontSize: 18, fill: '#94A3B8'}} />
                    <Area type="monotone" dataKey="count" stroke="#3b82f6" strokeWidth={6} fill="#1e3a8a" fillOpacity={0.4} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
